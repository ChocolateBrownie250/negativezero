import type { ApiNode } from './lib/tree';

export class UnauthorizedError extends Error {
  constructor() {
    super('unauthorized');
    this.name = 'UnauthorizedError';
  }
}

// Vite injects BASE_URL based on the `base` config (defaults to '/').
// We strip the leading '/' from the supplied path and concatenate so calls
// land at <base>api/... instead of /api/... — this is what lets the same
// client work whether served at the apex or under a path prefix like
// /services/basalt/. nginx in front strips the prefix before reaching the
// server, so the server still sees /api/... unchanged.
const API_BASE = import.meta.env.BASE_URL;

async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  const url = API_BASE + (path.startsWith('/') ? path.slice(1) : path);
  // Only declare a JSON content-type when there's actually a body. Fastify 5
  // rejects an empty body when content-type is application/json with
  // FST_ERR_CTP_EMPTY_JSON_BODY (400) — which broke every bodyless POST here
  // (passkey login/options, logout). Fastify 4 tolerated it.
  const headers: Record<string, string> = {
    ...((init.headers as Record<string, string>) ?? {}),
  };
  if (init.body != null) headers['content-type'] = 'application/json';
  const res = await fetch(url, {
    ...init,
    credentials: 'include',
    headers,
  });
  if (res.status === 401) throw new UnauthorizedError();
  const text = await res.text();
  let json: unknown = null;
  if (text) {
    try {
      json = JSON.parse(text);
    } catch {
      // non-json response
    }
  }
  if (!res.ok) {
    const err = new Error(
      (json && typeof json === 'object' && 'error' in json
        ? String((json as { error: unknown }).error)
        : 'request_failed') + ` (${res.status})`,
    );
    (err as Error & { status?: number; payload?: unknown }).status = res.status;
    (err as Error & { status?: number; payload?: unknown }).payload = json;
    throw err;
  }
  return json as T;
}

export const api = {
  me: () =>
    request<{ authenticated: boolean; hasPasskey: boolean }>('/api/auth/me'),
  logout: () => request<{ ok: true }>('/api/auth/logout', { method: 'POST' }),
  passkey: {
    loginOptions: () =>
      request<unknown>('/api/auth/passkey/login/options', { method: 'POST' }),
    loginVerify: (response: unknown) =>
      request<{ ok: true }>('/api/auth/passkey/login/verify', {
        method: 'POST',
        body: JSON.stringify({ response }),
      }),
    registerOptions: (
      args: { setupCode?: string; backupCode?: string } = {},
    ) =>
      request<unknown>('/api/auth/passkey/register/options', {
        method: 'POST',
        body: JSON.stringify(args),
      }),
    registerVerify: (response: unknown, deviceName?: string) =>
      request<{ ok: true; backupCode: string | null }>(
        '/api/auth/passkey/register/verify',
        {
          method: 'POST',
          body: JSON.stringify({ response, deviceName }),
        },
      ),
    list: () =>
      request<{
        credentials: Array<{
          id: string;
          deviceName: string | null;
          createdAt: number;
          lastUsed: number | null;
        }>;
      }>('/api/auth/passkey/list'),
    remove: (id: string) =>
      request<{ ok: true }>(
        `/api/auth/passkey/${encodeURIComponent(id)}`,
        { method: 'DELETE' },
      ),
  },
  rotateBackupCode: () =>
    request<{ backupCode: string }>('/api/auth/backup-code/rotate', {
      method: 'POST',
    }),
  listNodes: () => request<{ nodes: ApiNode[] }>('/api/nodes'),
  createNode: (
    payload:
      | { type: 'folder'; parentId: string; name: string }
      | { type: 'bookmark'; parentId: string; url: string; name?: string },
  ) =>
    request<{ node: ApiNode }>('/api/nodes', {
      method: 'POST',
      body: JSON.stringify(payload),
    }),
  patchNode: (
    id: string,
    payload: { name?: string; url?: string; parentId?: string; position?: number },
  ) =>
    request<{ node: ApiNode }>(`/api/nodes/${encodeURIComponent(id)}`, {
      method: 'PATCH',
      body: JSON.stringify(payload),
    }),
  deleteNode: (id: string) =>
    request<{ ok: true; deletedIds: string[] }>(
      `/api/nodes/${encodeURIComponent(id)}`,
      { method: 'DELETE' },
    ),
  importTree: (tree: unknown) =>
    request<{ ok: true; count: number }>('/api/import', {
      method: 'POST',
      body: JSON.stringify({ tree, mode: 'replace' }),
    }),
  reorderChildren: (parentId: string, orderedIds: string[]) =>
    request<{ ok: true }>('/api/nodes/reorder', {
      method: 'POST',
      body: JSON.stringify({ parentId, orderedIds }),
    }),
};
